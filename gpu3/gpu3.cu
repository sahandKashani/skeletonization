...
int skeletonize(Bitmap** src_bitmap, Bitmap** dst_bitmap, dim3 grid_dim, dim3 block_dim) {
    ...
    int iterations = 0;
    do {
        int skeletonize_pass_shared_mem_size = (block_dim.x + PAD_LEFT + PAD_RIGHT) * (block_dim.y + PAD_TOP + PAD_BOTTOM);
...
__global__ void skeletonize_pass(uint8_t* d_src, uint8_t* d_dst, int g_width, int g_height) {
    // shared memory for tile
    extern __shared__ uint8_t s_data[];
    ...
    if ((g_row < g_height) & (g_col < g_width)) {
        // load data into shared memory tile

        // "center" pixels
        s_data[s_row * s_width + s_col] = d_src[g_row * g_width + g_col];

        // top & bottom "padding" pixels
        if (s_row == PAD_TOP) {

        } else if (s_row == (s_height - 1)) {
        ...
        // continue skeletonization
        uint8_t NZ = black_neighbors_around(d_src, g_row, g_col, g_width, g_height);
        uint8_t TR_P1 = wb_transitions_around(d_src, g_row, g_col, g_width, g_height);
        uint8_t TR_P2 = wb_transitions_around(d_src, g_row - 1, g_col, g_width, g_height);
        uint8_t TR_P4 = wb_transitions_around(d_src, g_row, g_col - 1, g_width, g_height);
        uint8_t P2 = P2_f(d_src, g_row, g_col, g_width, g_height);
        uint8_t P4 = P4_f(d_src, g_row, g_col, g_width, g_height);
        uint8_t P6 = P6_f(d_src, g_row, g_col, g_width, g_height);
        uint8_t P8 = P8_f(d_src, g_row, g_col, g_width, g_height);
        ...
        d_dst[g_row * g_width + g_col] = BINARY_WHITE + ((1 - thinning_cond_ok) * d_src[g_row * g_width + g_col]);
    }
}
